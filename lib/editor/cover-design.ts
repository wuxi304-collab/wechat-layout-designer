...
export function buildCoverHarness(title: string, companyName: string | null) {
  ...
  return {
    workflowVersion: "5.0",
    rule: "NO_ASSET_NO_COMPOSE",
    company: {
      company: companyName,
      officialDomainHints: companyRecord?.officialDomains ?? [],
    },
    steps: companyName ? [
      "素材 Agent 只返回官网或认证微信公众号取得的官方 Logo 文件，以及一张无字无 Logo 底图",
      "在本页分别导入底图和 Logo；缺少任一真实文件，最终封面按钮保持禁用",
      "由本机 Canvas 原样放置 Logo，并精确排印锁定标题与署名后下载 PNG",
    ] : [
      "素材 Agent 只返回一张无字、无第三方 Logo 的封面底图",
      "在本页导入底图；没有真实文件，最终封面按钮保持禁用",
      "由本机 Canvas 精确排印锁定标题与署名后下载 PNG",
    ],
    requiredAssets: companyName ? ["BACKGROUND_IMAGE", "OFFICIAL_LOGO_IMAGE"] : ["BACKGROUND_IMAGE"],
    composeOnlyWhen: companyName ? "ALL_REQUIRED_LOCAL_FILES_SELECTED" : "BACKGROUND_IMAGE_SELECTED",
    stopWhen: companyName ? "BACKGROUND_OR_OFFICIAL_LOGO_FILE_MISSING" : "BACKGROUND_IMAGE_MISSING",
    finalization: "BROWSER_CANVAS_DETERMINISTIC_COMPOSITE",
    lockedText: {
...
export function buildCoverPrompt(style: CoverStyle, title: string, profile: CoverArticleProfile, companyName: string | null = profile.companyName) {
  ...
  const brandSteps = companyName ? [
    ...
    `1. 先打开${companyName}官方网站，下载当前页面实际使用的 Logo 图片；官网没有可用图片时，再从该企业认证微信公众号头像、菜单页或官方文章页取得。`,
    "2. 必须返回一个真实可下载的 PNG、SVG、WebP 或 JPG 文件，并确认该文件已存在于任务附件或本地路径。网页链接、企业名称、文字描述和搜索缩略图都不算文件。",
    "3. 不得让图像模型猜、画、描摹或仿制 Logo；不得用百科、自媒体、素材站或搜索结果缩略图代替官方文件。",
    "4. 找不到官方 Logo 文件时，立即停止并只回复“未取得官方 Logo 文件，请用户上传”；不要继续生成底图。",
    "5. 返回 Logo 文件时，另附一行官方来源网址，供用户复核。",
  ] : [
    "【素材 A｜Logo】",
    "文章未识别到企业主体：不需要 Logo 文件，也不得凭空创造品牌标志。",
  ];

  return [
    "【封面素材准备任务｜Asset Pack V5】",
    "你是素材检索与无字底图制作 Agent。本任务只准备素材，不制作、不排版、不得交付最终封面。",
    companyName ? "必须只返回两个实际图片文件：A. 官方 Logo 原图；B. 无字无 Logo 的横幅底图。只返回底图视为任务失败。" : "只返回一个实际图片文件：无字无 Logo 的横幅底图。",
    ...brandSteps,
    "【素材 B｜无字底图】",
    "生成一张 2.35:1 的微信公众号横幅底图，建议 2350×1000。底图中禁止出现标题、署名、Logo、占位框、乱码、伪中文和任何文字。",
    "左侧约 58% 保持低纹理、可读的标题安全区；右上保留 Logo 安全区；右下保留署名安全区。只设计光影、材质和主体关系，不画边框或文字占位符。",
    "不要 moodboard、样机、设计说明、过程图、四宫格、方案板或带界面的预览图。",
    "【画面设计】",
    ...
    "【后续锁定文字｜仅供站点合成参考，严禁画入底图】",
    resolvedTitle ? `主标题：“${resolvedTitle}”` : "缺少主标题：停止任务并向用户索取，不得自拟。",
    `固定署名：“${coverSignature}”`,
    "标题、署名与 Logo 均由排版站本机 Canvas 合成；图像模型不得书写、仿制或提前放置。",
    "【素材包验收】",
    companyName ? `1. 附件中真实存在${companyName}官方 Logo 图片文件，并附官方来源网址。` : "1. 没有任何企业 Logo。",
    "2. 附件中真实存在一张 2.35:1 无字、无 Logo 底图。",
    companyName ? "3. 两个文件缺少任意一个都不得声称完成；只返回底图视为任务失败。" : "3. 底图文件不存在时不得声称完成。",
    "4. 不要输出最终封面；用户会把素材导入排版站，由本机 Canvas 合成。",
  ].join("\n");
}
