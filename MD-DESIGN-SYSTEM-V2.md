...
| Han.css | 语义化中文排印、简繁日回退 | 文章容器的语言、标点与回退边界 |
| typo.css | CJK 断行、自动间距、标点挤压 | 渐进增强的中文混排规则 |
| LXGW WenKai | 屏幕文楷与开放字库边界 | 仅作已安装字体回退，不捆绑字体文件 |
| Sarasa Gothic | 中西文等宽组合与 UI/Mono 分工 | Markdown 原稿与代码块字体链 |
...
3. UI 使用无衬线，标题使用宋/楷体系，正文按刊宋、文楷、屏读黑三类语义槽位切换，代码使用中西文等宽字体。
4. 状态优先用边线、字重和位置表达，避免标签堆叠。
5. 主题色不超过三类：强调、正文、承载面。
6. 微信复制结果不得依赖 class、外链 CSS、动画、定位或伪元素。
7. 深色预览按标题、正文、弱文字、边线、承载面分别映射。
8. 中文排印使用 `lang="zh-CN"`、严格断行与渐进式标点压缩；中文标签不使用英文式大字距。
9. 字体文件不从第三方仓库直接搬运。先使用系统字体回退链；引入 Web Font 前必须核对许可证、字库体积、子集规则与微信端失效策略。
