...
export default function Home() {
  ...
  async function copyCoverPrompt() {
    try {
      ...
      notify(`“${selectedCoverLabel.name}”封面生产协议 V3 已复制`);
    } catch {
      ...
      <section className="canvas-area">
        ...
        <div className={`canvas-stage ${preview}`} onClick={() => setSelected(null)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const snippet = event.dataTransfer.getData("text/plain"); if (snippet) insertComponent(snippet, "语义"); }}>
          ...
          {stage === "封面" ? <section className="cover-workbench" onClick={(event) => event.stopPropagation()} style={{ "--cover-paper": selectedCoverStyle.palette[0], "--cover-ink": selectedCoverStyle.palette[1], "--cover-accent": selectedCoverStyle.palette[2] } as React.CSSProperties}>
            <header className="cover-workbench-head"><div><span>Prompt Protocol / V3</span><h2>{selectedCoverLabel.name}</h2><p>{selectedCoverStyle.direction}</p></div><strong><b>{coverRecommendations.find((item) => item.style.id === selectedCoverStyle.id)?.score ?? 88}</b><small>匹配度</small></strong></header>
            ...
            <div className="cover-protocol-strip"><header><span>六段视觉协议</span><small>风格服从事实，交付服从闸门</small></header><div><p><em>01</em><span><b>模板</b><small>{coverProtocol.template}</small></span></p><p><em>02</em><span><b>主锚点</b><small>{coverProtocol.visualAnchor}</small></span></p><p><em>03</em><span><b>唯一隐喻</b><small>{coverProtocol.visualMetaphor}</small></span></p><p><em>04</em><span><b>镜头</b><small>{coverProtocol.shotLanguage}</small></span></p><p><em>05</em><span><b>层级</b><small>{coverProtocol.hierarchy}</small></span></p><p><em>06</em><span><b>禁区</b><small>{coverProtocol.negativeLock[0]}</small></span></p></div></div>
            ...
            <footer className="cover-workbench-foot"><div className="cover-palette"><span>配色</span>{selectedCoverStyle.palette.map((color) => <i key={color} style={{ background: color }} title={color}/>)}</div><button onClick={copyCoverPrompt}><Icon name="copy" size={15}/>复制生产协议 V3</button></footer>
      ...
      <aside className="inspector-panel" ref={inspectorPanelRef}>
        ...
        {inspector === "封面" && <div className="inspector-content cover-inspector">
          ...
          <section className={`cover-brand-card ${effectiveCoverCompany ? "enterprise" : "generic"}`}>
            <div className="section-heading"><div><span>企业与官方 Logo</span><small>实体先锁定，Logo 原件再入场</small></div><span className="cover-brand-state">{effectiveCoverCompany ? "实体已锁定" : "未启用"}</span></div>
            ...
            <p><b>{effectiveCoverCompany ? `已锁定：${effectiveCoverCompany}` : "当前按非企业主题处理"}</b><span>{effectiveCoverCompany ? `${coverCompanyOverride?.source === coverCompanySource ? "人工指定企业主体" : coverProfile.companyReason ?? "标题实体识别"}；Logo 仍须取得官方原始文件、来源 URL 与 SHA-256，缺一项便阻断交付。` : "不会擅自植入第三方 Logo；若文章实际写企业，请在上方补全主体。"}</span></p>
          </section>
          <section className="inspector-section cover-harness-card">
            <div className="section-heading"><div><span>交付闸门 Harness V3</span><small>不是提醒，是不通过便不交付</small></div><span className="protocol-badge">4 GATES</span></div>
            <ul className="cover-harness-gates">
              <li className={effectiveCoverCompany ? "pass" : "idle"}><Icon name={effectiveCoverCompany ? "check" : "warning"} size={15}/><span><b>企业实名</b><small>{effectiveCoverCompany || "非企业稿 / 等待指定"}</small></span><em>{effectiveCoverCompany ? "LOCKED" : "N/A"}</em></li>
              <li className={effectiveCoverCompany ? "blocked" : "idle"}><Icon name={effectiveCoverCompany ? "warning" : "check"} size={15}/><span><b>Logo 原件</b><small>{effectiveCoverCompany ? `须来自官方域名；${coverHarness.entityGate.officialDomainHints.join(" / ") || "执行时联网核验"}` : "无需企业 Logo"}</small></span><em>{effectiveCoverCompany ? "BLOCK" : "N/A"}</em></li>
              <li className="pass"><Icon name="check" size={15}/><span><b>确定性排字</b><small>标题与“钢铁私塾 唐淼”不再交给生图模型</small></span><em>LOCKED</em></li>
              <li className="blocked"><Icon name="warning" size={15}/><span><b>成品验收</b><small>OCR 精确相等、无额外文字、Logo 指纹一致</small></span><em>REQUIRED</em></li>
            </ul>
          </section>
          <section className="inspector-section cover-protocol-card"><div className="section-heading"><div><span>Prompt as Code V3</span><small>视觉协议、资产闸门与验收契约</small></div><span className="protocol-badge">HARNESS</span></div><ol><li><i>01</i><span><b>模板</b><small>{coverProtocol.template}</small></span></li><li><i>02</i><span><b>主锚点</b><small>{coverProtocol.visualAnchor}</small></span></li><li><i>03</i><span><b>视觉隐喻</b><small>{coverProtocol.visualMetaphor}</small></span></li><li><i>04</i><span><b>镜头纪律</b><small>{coverProtocol.shotLanguage}</small></span></li><li><i>05</i><span><b>信息层级</b><small>{coverProtocol.hierarchy}</small></span></li><li><i>06</i><span><b>参考资产</b><small>{coverProtocol.referencePolicy}</small></span></li></ol><div className="cover-protocol-tags">{coverProtocol.styleTags.map((tag) => <span key={tag}>{tag}</span>)}</div></section>
          ...
          <section className="inspector-section cover-selected-card"><div className="section-heading"><div><span>当前设计建议</span><small>{selectedCoverLabel.short} · {selectedCoverStyle.fit}</small></div></div><dl><div><dt>标题</dt><dd>最终成品必须逐字排印“{article.title}”，并由确定性文字层完成。</dd></div><div><dt>构图</dt><dd>{selectedCoverStyle.composition}</dd></div><div><dt>材质</dt><dd>{selectedCoverStyle.texture}</dd></div><div><dt>避坑</dt><dd>{selectedCoverStyle.avoid}</dd></div></dl><button onClick={copyCoverPrompt}><Icon name="copy" size={15}/>复制生产协议 V3</button></section>
