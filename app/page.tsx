...
export default function Home() {
  ...
  async function copyCoverPrompt() {
    try {
      ...
      notify(`“${selectedCoverLabel.name}”精准封面任务 V7 已复制`);
    } catch {
      ...
      <section className="canvas-area">
        ...
        <div className={`canvas-stage ${preview}`} onClick={() => setSelected(null)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const snippet = event.dataTransfer.getData("text/plain"); if (snippet) insertComponent(snippet, "语义"); }}>
          ...
          {stage === "封面" ? <section className="cover-workbench" onClick={(event) => event.stopPropagation()} style={{ "--cover-paper": selectedCoverStyle.palette[0], "--cover-ink": selectedCoverStyle.palette[1], "--cover-accent": selectedCoverStyle.palette[2] } as React.CSSProperties}>
            <header className="cover-workbench-head"><div><span>Asset → Image → Compose / V7</span><h2>{selectedCoverLabel.name}</h2><p>{selectedCoverStyle.direction}</p></div><strong><b>{coverRecommendations.find((item) => item.style.id === selectedCoverStyle.id)?.score ?? 88}</b><small>匹配度</small></strong></header>
      ...
      <aside className="inspector-panel" ref={inspectorPanelRef}>
        ...
        {inspector === "封面" && <div className="inspector-content cover-inspector">
          ...
          <section className="inspector-section cover-composer-card">
            <div className="section-heading"><div><span>精准封面任务</span><small>四步执行：素材、底图、合成、验收</small></div><span className="protocol-badge ready">V7 SHORT</span></div>
            <button className="cover-asset-task" onClick={copyCoverPrompt}><Icon name="copy" size={15}/><span><b>复制精准封面任务 V7</b><small>短指令，只交付 final-cover.png</small></span></button>
            <div className="cover-fallback-heading"><b>本机合成兜底</b><small>仅当外部 Agent 明确不具备确定性排版能力时使用</small></div>
          ...
          <section className="inspector-section cover-selected-card"><div className="section-heading"><div><span>当前设计建议</span><small>{selectedCoverLabel.short} · {selectedCoverStyle.fit}</small></div></div><dl><div><dt>标题</dt><dd>最终成品必须逐字排印“{article.title}”，并由确定性文字层完成。</dd></div><div><dt>构图</dt><dd>{selectedCoverStyle.composition}</dd></div><div><dt>材质</dt><dd>{selectedCoverStyle.texture}</dd></div><div><dt>避坑</dt><dd>{selectedCoverStyle.avoid}</dd></div></dl><button onClick={copyCoverPrompt}><Icon name="copy" size={15}/>复制精准封面任务 V7</button></section>
